...
# Mirror of `is_interval_like` in the mz-sql-anonymize helper. The helper keeps
...
# a bare datetime-field keyword (a `date_trunc`/`date_part` argument). A bare
# unit word with no magnitude (`'y'`, `'min'`) is data and IS redacted/flagged.
...
def is_interval_like(s: str) -> bool:
    ...
    if s.lower() in _DATE_FIELDS:
        return True
    # Otherwise a duration must carry a numeric magnitude; a bare unit word is
    # data and gets redacted.
    saw_number = False
    for word in s.split():
        word = word.lower()
        if _is_number(word):
...
def verify_anonymized(
    ...
    def is_anonymized_literal(literal: str) -> bool:
        ...
        return (
            ...
            # The value part of an interval literal (`INTERVAL '60' DAY`) is kept
            # whole by the helper. A bare-number string only ever survives this
            # way — a `Value::String` number is always redacted — so it is safe.
            or re.fullmatch(r"-?[0-9]+(\.[0-9]+)?", content) is not None
